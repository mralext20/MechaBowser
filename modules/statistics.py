...
class StatCommands(commands.Cog, name='Statistic Commands'):
    ...
    async def _stats_server(self, ctx, start_date=None, end_date=None):
        msg = await ctx.send(
            'One moment, crunching message and channel data...')

        try:
            searchDate = datetime.datetime.utcnow(
            ) if not start_date else datetime.datetime.strptime(
                start_date, '%Y-%m-%d').replace(tzinfo=pytz.UTC)
            searchDate = searchDate.replace(hour=0, minute=0, second=0)
            endDate = searchDate + datetime.timedelta(
                days=30) if not end_date else datetime.datetime.strptime(
            ...
            return await msg.edit(
                content=
                f'{config.redTick} Invalid date provided. Please make sure it is in the format of `yyyy-mm-dd`'
            )

        if not start_date:
            messages = mclient.bowser.messages.find({
                'timestamp': {
                    '$gte': (int(time.time()) - (60 * 60 * 24 * 30))
                }
            })
            ...
            if endDate <= searchDate:
                return await msg.edit(
                    content=
                    f'{config.redTick} Invalid dates provided. The end date is before the starting date. `{ctx.prefix}stats server [starting date] [ending date]`'
                )

            messages = mclient.bowser.messages.find({
                'timestamp': {
                    '$gte': searchDate.timestamp(),
        ...
        topChannels = sorted(
            channelCounts.items(), key=lambda x: x[1], reverse=True
        )[0:
          5]  # Get a list of tuple sorting by most active channel to least, and only include top 5
        topChannelsList = []
        for x in topChannels:
            channelObj = self.bot.get_channel(x[0])
            if channelObj:
                topChannelsList.append(
                    f'{self.bot.get_channel(x[0]).mention} ({x[1]})')
        ...
        for member in mclient.bowser.users.find({'joins': {'$ne': []}}):
            for join in member['joins']:
                if not start_date and (
                        searchDate.timestamp() -
                    (60 * 60 * 24 * 30)) <= join <= endDate.timestamp():
                    netJoins += 1

                elif start_date and searchDate.timestamp(
                ) <= join <= endDate.timestamp():
                    netJoins += 1

            for leave in member['leaves']:
                if not start_date and (
                        searchDate.timestamp() -
                    (60 * 60 * 24 * 30)) <= leave <= endDate.timestamp():
                    netLeaves += 1

                elif start_date and searchDate.timestamp(
                ) <= leave <= endDate.timestamp():
                    netLeaves += 1

        activeChannels = ', '.join(topChannelsList)
        premiumTier = 'No tier' if ctx.guild.premium_tier == 0 else f'Tier {ctx.guild.premium_tier}'

        dayStr = 'In the last 30 days' if not start_date else 'Between ' + searchDate.strftime(
    ...
    async def _stats_users(self, ctx):
        ...
        topSenders = sorted(
            msgCounts.items(), key=lambda x: x[1], reverse=True
        )[0:
          25]  # Get a list of tuple sorting by most message to least, and only include top 25
        embed = discord.Embed(
            title='Top User Statistics',
            description=
            'List of the 25 highest message senders and their count during the last 30 days\n',
            color=0xD267BA)
        for x in topSenders:
    ...
    @commands.has_any_role(config.moderator, config.eh)
    async def _stats_roles(self, ctx, *, role: typing.Optional[typing.Union[
        discord.Role, int,
        str]]):  # TODO: create and pull role add/remove data from events
        ...
        if role:
            if type(role) is int:
                ...
                if not role:
                    return await ctx.send(
                        f'{config.redTick} There is no role by that ID')

            elif type(role) is str:
                role = discord.utils.get(ctx.guild.roles, name=role)
                if not role:
                    return await ctx.send(
                        f'{config.redTick} There is no role by that name')
            ...
            embed = discord.Embed(
                title=f'{ctx.guild.name} Role Statistics',
                description=
                f'Server role list and respective member count\n\n{roleList}',
                color=0xD267BA)
    ...
    async def _stats_channels(self, ctx):
        return await ctx.send(
            f'{config.redTick} Channel statistics are not ready for use')
