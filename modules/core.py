...
class MainEvents(commands.Cog):
    ...
    async def sanitize_eud(self):
        ...
        msgDB.update_many({'timestamp': {"$lte": time.time() - (86400 * 30)},
                           'sanitized': False}, {"$set": {'content': None, 'sanitized': True}})
    ...
    async def on_member_join(self, member):
        ...
        if not doc:
            ...
            db.update_one({'_id': member.id}, {'$push': {'joins': (
                datetime.datetime.utcnow() - datetime.datetime.utcfromtimestamp(0)).total_seconds()}})

        new = ':new: ' if (datetime.datetime.utcnow(
        ) - member.created_at).total_seconds() <= 60 * 60 * 24 * 14 else ''  # Two weeks

        #log = f':inbox_tray: {new} User **{str(member)}** ({member.id}) joined'

        embed = discord.Embed(
            color=0x417505, timestamp=datetime.datetime.utcnow())
        ...
        if restored:
            ...
            puns = mclient.bowser.puns.find(
                {'user': member.id, 'active': True})
            restoredPuns = []
            if puns.count():
                for x in puns:
                    if x['type'] == 'blacklist':
                        restoredPuns.append(
                            punTypes[x['type']].format(x['context']))
            ...
            embed = discord.Embed(
                color=0x4A90E2, timestamp=datetime.datetime.utcnow())
            embed.set_author(
        ...
        if 'migrate_unnotified' in doc.keys() and doc['migrate_unnotified'] == True:
            # Should only be one, it's mutually exclusive
            for pun in mclient.bowser.puns.find({'active': True, 'type': {'$in': ['tier1', 'tier2', 'tier3']}, 'user': member.id}):
                strikeCount = int(pun['type'][-1:]) * 4

                mclient.bowser.puns.update_one(
                    {'_id': pun['_id']}, {'$set': {'active': False}})
                ...
                db.update_one({'_id': member.id}, {'$set': {'migrate_unnotified': False, 'strike_check': time.time(
                ) + (60 * 60 * 24 * 7)}})  # Setting the next expiry check time
                mod = self.bot.get_cog('Moderation Commands')
                ...
                explanation = ('Hello there **{}**,\nI am letting you know of a change in status for your active level {} warning issued on {}.\n\n'
                    ...
                    datetime.datetime.utcfromtimestamp(
                        pun['timestamp']).strftime('%B %d, %Y'),  # Date of warn
    ...
    async def on_member_remove(self, member):
        ...
        if puns.count():
            ...
            embed = discord.Embed(
                color=0x8B572A, timestamp=datetime.datetime.utcnow())
    ...
    async def on_member_ban(self, guild, user):
        ...
        embed = discord.Embed(color=discord.Color(
            0xD0021B), timestamp=datetime.datetime.utcnow())
    ...
    async def on_member_unban(self, guild, user):
        ...
        embed = discord.Embed(color=discord.Color(
            0x88FF00), timestamp=datetime.datetime.utcnow())
        embed.set_author(name=f'{user} ({user.id})', icon_url=user.avatar_url)
        embed.add_field(name='Mention', value=f'<@{user.id}>')
    ...
    async def on_bulk_message_delete(self, messages):
        ...
        embed = discord.Embed(
            description=f'Archive URL: {config.baseUrl}/logs/{archiveID}', color=0xF5A623, timestamp=datetime.datetime.utcnow())
    ...
    async def on_message_delete(self, message):
        ...
        embed.set_author(
            name=f'{str(message.author)} ({message.author.id})', icon_url=message.author.avatar_url)
        ...
        if len(message.attachments) == 1:
            ...
            for a in range(len(attachments)):
                embed.add_field(
                    name=f'Attachment {a + 1}', value=attachments[a])

        if len(content) > 1950:  # Too long to safely add jump to message in desc, use field
            embed.description = content
            embed.add_field(
                name='Jump', value=f'[Jump to message]({message.jump_url})')
    ...
    async def on_message_edit(self, before, after):
        ...
        if len(before.content) <= 1024 and len(after.content) <= 1024:
            embed = discord.Embed(
                description=f'[Jump to message]({before.jump_url})', color=0xF8E71C, timestamp=datetime.datetime.utcnow())
        ...
        embed.set_author(
            name=f'{str(before.author)} ({before.author.id})', icon_url=before.author.avatar_url)
    ...
    async def on_member_update(self, before, after):
        ...
        if before.nick != after.nick:
            ...
            embed = discord.Embed(
                color=0x9535EC, timestamp=datetime.datetime.utcnow())
            embed.set_author(
                name=f'{before} ({before.id})', icon_url=before.avatar_url)
        ...
        if before.roles != after.roles:
            ...
            if rolesRemoved or rolesAdded:  # nop if no change, e.g. role moves in list
                ...
                embed.add_field(name=f'Current Role{"" if len(roleStr) == 1 else "s"}', value=', '.join(
                    n for n in reversed(roleStr)), inline=False)
                embed.add_field(name='Mention', value=f'<@{before.id}>')
    ...
    async def _pundb(self, ctx, _type, user, moderator, strTime, active: typing.Optional[bool], *, reason='-No reason specified-'):
        ...
        expiry = None if not active else int(
            date.timestamp() + (60 * 60 * 24 * 30))
