...
class SocialFeatures(commands.Cog, name='Social Commands'):
    ...
    async def _profile(self, ctx, member: typing.Optional[discord.Member]):
        ...
        if not dbUser['profileSetup'] and member == ctx.author and ctx.guild.get_role(
                config.moderator
        ) not in ctx.author.roles and ctx.channel.id != config.commandsChannel:
    ...
    async def _generate_profile_card(self,
        ...
        if guild.get_role(
                config.modemeritus) in member.roles or guild.get_role(
    ...
    async def _profile_edit(self, ctx):
        ...
        if ctx.guild.get_role(
                config.moderator
        ) not in ctx.author.roles and ctx.channel.id != config.commandsChannel:  # commands
            await ctx.message.delete()
            return await ctx.send(
                f'{config.redTick} {ctx.author.mention} Please use bot commands in <#{config.commandsChannel}>, not {ctx.channel.mention}',
                delete_after=15)

        if ctx.author.id in self.inprogressEdits.keys() and (
                time.time() - self.inprogressEdits[ctx.author.id]) < 300:
            await ctx.message.delete()
            return await ctx.send(
        ...
        header = {
            True:
            f'{headerBase} If you would like to instead reset a section of your profile that you have previously set, just respond `reset` to any prompt.\n\n',
            False: f'{headerBase}\n\n'
        }
        ...
        embedText = {
            ...
            'descBase': {
                True:
                'Welcome back to profile setup.',
                False:
                'It appears you have not setup your profile before, let\'s get that taken care of!'
            }
        }
        ...
        async def _phase1(message):
            ...
            if response.content.lower().strip() == 'reset':
                ...
                await message.channel.send(
                    'I\'ve gone ahead and reset your setting for **friend code**'
                )
                return True
        ...
        async def _phase2(message):
            ...
            for x in rawPoints:
                if str(
                        hex(x)[2:]
                ) not in self.letterCodepoints:  # Flags are the 2 letter abbrev. in regional letter emoji
                    return False
        ...
        async def _phase3(message):
            ...
            if response.content.lower().strip() == 'reset':
                ...
                await message.channel.send(
                    'I\'ve gone ahead and reset your setting for **timezone**')
                return True
        ...
        async def _phase4(message):
            ...
            while gameCnt < 3:
                ...
                if response.content.lower().strip() == 'reset':
                    ...
                    await message.channel.send(
                        'I\'ve gone ahead and reset your setting for **favorite games**'
                    )
                    return True
                ...
                if results and results[0][1] >= 86:
                    ...
                    while True:
                        ...
                        checkResp = await self.bot.wait_for('message',
                                                            timeout=120,
                                                            check=check)
        ...
        try:
            ...
            return await mainMsg.channel.send(
                'You are all set! Your profile has been edited:', file=card)
            ...
            return await botMsg.edit(
                content=
                f'{ctx.author.mention} You have taken too long to respond and the edit has been timed out, please run `!profile edit` to start again'
            )
    ...
    async def on_message(self, message):
        ...
        if message.channel.id not in [
                config.commandsChannel, config.voiceTextChannel
        ]:
            await message.channel.send(
    ...
    async def social_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            if cmd_str == 'profile' and (ctx.message.channel.id in [
                    config.commandsChannel, config.voiceTextChannel
            ] or ctx.guild.get_role(config.moderator) in ctx.author.roles):
                await self._profile.__call__(
                    ctx, None if not ctx.args else ctx.args[0])
            else:
                return await ctx.send(
