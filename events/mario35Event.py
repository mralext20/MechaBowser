...
class MarioGame(commands.Cog):
    ...
    async def _pricepost(self, ctx):
        ...
        embed.add_field(
            name="How do I buy something?",
            value='Easy! Run the `!buy item` command in this channel, replacing "item" with what you would like to buy. The item name is the highlighted section for each item in the store. For example, if you wanted to buy the **Super Mario 3D All-stars Profile Background**, run `!buy allstars-profile`.',
        )
    ...
    async def _buy(self, ctx, item):
        ...
        if ctx.channel.id != 757411216774791189:
            return await ctx.send(
                f"{config.redTick} {ctx.author.mention} You can only use this command in <#757411216774791189>",
                delete_after=10,
            )
        ...
        if not status:
            return await ctx.send(
                f"{config.redTick} {ctx.author.mention} You don't have any coins yet! Keep a look out for coin messages in <#238081280632160257>, <#238081135865757696>, and <#325430144993067049> to get some before trying to buy items!"
            )

        if item not in items:
            return await ctx.send(
                f'{config.redTick} {ctx.author.mention} Hmm, I don\'t recognize "{item}" as an item for sale',
                delete_after=10,
            )
        ...
        if item[-7:] == "profile":
            ...
            if item[:-8] in user["backgrounds"]:
                return await ctx.send(
                    f"{config.redTick} {ctx.author.mention} You already have that background! To set it as your background, use the `!profile edit` command in <#670999043740270602>",
                    delete_after=10,
                )

            if status["coins"] < 20:
                return await ctx.send(
                    f"{config.redTick} {ctx.author.mention} You do not have enough coins to get that!", delete_after=10
                )

            event.update_one({"_id": ctx.author.id}, {"$inc": {"coins": -20}})
            db.update_one({"_id": ctx.author.id}, {"$push": {"backgrounds": item[:-8]}})
            return await ctx.send(
                f"{config.greenTick} {ctx.author.mention} Success! You bought the **{item[:-8]} profile background**. To set it as your current background, use the `!profile edit` command in <#670999043740270602>",
                delete_after=20,
            )
            ...
            try:
                ...
                if entered:
                    return await ctx.send(
                        f"{config.redTick} {ctx.author.mention} You already have an entry ticket to the giveaway!",
                        delete_after=10,
                    )
            ...
            return await ctx.send(
                f"{config.greenTick} {ctx.author.mention} Success! You bought a **giveaway raffle ticket**.",
                delete_after=10,
            )
    ...
    async def _group_points(self, ctx):
        if ctx.channel.id != 758418138269483081:
            await ctx.message.delete()
            return await ctx.send(
                f"{config.redTick} {ctx.author.mention} You cannot use that command here! Try it in <#758418138269483081> instead",
                delete_after=10,
            )
        ...
        await ctx.send(
            f'{ctx.author.mention}, You have {points["coins"]} coins and are #{placing[0]} on the leaderboard! Keep up the good work, and hit more blocks!'
        )
    ...
    async def _group_leaderboard(self, ctx):
        ...
        embed = Embed(
            title="Leaderboard", description="Here is the current standing for all block punchers", color=0x4A90E2
        )
        ...
        if maxEntries == 10:
            ...
            embed.add_field(
                name="Scores",
                value="Hm, it looks like no one has a score yet. User scores will show up here after at least one person scores a point. Keep an eye out for question blocks!",
            )
    ...
    async def on_message(self, message):
        ...
        gameMessage = await message.channel.send(
            "It's a me, Mario! Keep reacting with <:mariocoin:757300710894207067> to get upto 5 coins from the question block!",
            embed=embed,
        )
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if reaction.message.id in self.gameMessages.keys() and reaction.emoji.id == 757300710894207067:
            ...
            print("valid")
            if (
                user.id in self.gameMessages[reaction.message.id].keys()
                and self.gameMessages[reaction.message.id][user.id] < 5
            ):
    ...
    async def end_game(self, message):
        ...
        for key, value in self.gameMessages[message.id].items():
            db.update_one({"_id": key}, {"$inc": {"coins": value}}, upsert=True)
