...
class Moderation(commands.Cog, name='Moderation Commands'):
    def __init__(self, bot):
        ...
        pendingLogs = db.find(
            {'public': True, 'public_log_message': None, 'type': {'$ne': 'note'}})
        loop = bot.loop
        for log in pendingLogs:
            ...
            loop.create_task(tools.send_public_modlog(
                bot, log['_id'], self.publicModLogs, expires))

        # Run expiration tasks
        userDB = mclient.bowser.users
        pendingPuns = db.find(
            {'active': True, 'type': {'$in': ['strike', 'mute']}})
        twelveHr = 60 * 60 * 12
        ...
        for pun in pendingPuns:
            if pun['type'] == 'strike':
                ...
                if user['strike_check'] > time.time():  # In the future
                    tryTime = twelveHr if user['strike_check'] - time.time(
                    ) > twelveHr else user['strike_check'] - time.time()
                    self.taskHandles.append(self.bot.loop.call_later(
                        tryTime, asyncio.create_task, self.expire_actions(pun['_id'], config.nintendoswitch)))

                else:  # In the past
                    self.taskHandles.append(self.bot.loop.call_soon(
                        asyncio.create_task, self.expire_actions(pun['_id'], config.nintendoswitch)))

            elif pun['type'] == 'mute':
                tryTime = twelveHr if pun['expiry'] - \
                    time.time() > twelveHr else pun['expiry'] - time.time()
    ...
    async def _hide_modlog(self, ctx, uuid):
        ...
        # Toggle sensitive value
        sensitive = True if not doc['sensitive'] else False
        ...
        if not doc['public_log_message']:
            ...
            assert embedDict['fields'] != newEmbedDict['fields']
            db.update_one({'_id': uuid}, {'$set': {
                'sensitive': sensitive
            }})
    ...
    async def _banning(self, ctx, users: commands.Greedy[ResolveUser], *, reason='-No reason specified-'):
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Ban reason is too long, reduce it by at least {len(reason) - 990} characters')
        if not users:
    ...
    async def _unbanning(self, ctx, user: int, *, reason='-No reason specified-'):
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Unban reason is too long, reduce it by at least {len(reason) - 990} characters')
        db = mclient.bowser.puns
        ...
        openAppeal = mclient.modmail.logs.find_one(
            {'open': True, 'ban_appeal': True, 'recipient.id': str(user)})
        if openAppeal:
    ...
    async def _kicking(self, ctx, member: discord.Member, *, reason='-No reason specified-'):
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Kick reason is too long, reduce it by at least {len(reason) - 990} characters')
    ...
    async def _muting(self, ctx, member: discord.Member, duration, *, reason='-No reason specified-'):
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Mute reason is too long, reduce it by at least {len(reason) - 990} characters')
        db = mclient.bowser.puns
        ...
        tryTime = twelveHr if expireTime - \
            time.time() > twelveHr else expireTime - time.time()
        self.taskHandles.append(self.bot.loop.call_later(
            tryTime, asyncio.create_task, self.expire_actions(docID, ctx.guild.id)))
        if await tools.mod_cmd_invoke_delete(ctx.channel):
            return await ctx.message.delete()
    ...
    async def _unmuting(self, ctx, member: discord.Member, *, reason='-No reason specified-'):
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Unmute reason is too long, reduce it by at least {len(reason) - 990} characters')
        db = mclient.bowser.puns
        muteRole = ctx.guild.get_role(config.mute)
    ...
    async def _strike(self, ctx, member: discord.Member, count: typing.Optional[StrikeRange] = 1, *, reason):
        ...
        if len(reason) > 990:
            return await ctx.send(f'{config.redTick} Strike reason is too long, reduce it by at least {len(reason) - 990} characters')
        ...
        self.taskHandles.append(self.bot.loop.call_later(60 * 60 * 12, asyncio.create_task,
                                                         self.expire_actions(docID, ctx.guild.id)))  # Check in 12 hours, prevents time drifting
    ...
    async def _strike_set(self, ctx, member: discord.Member, count: StrikeRange, *, reason):
        ...
        activeStrikes = 0
        puns = punDB.find(
            {'user': member.id, 'type': 'strike', 'active': True})
        for pun in puns:
            activeStrikes += pun['active_strike_count']
        ...
        if activeStrikes == count:
            ...
            puns = punDB.find({'user': member.id, 'type': 'strike', 'active': True}).sort(
                'timestamp', 1)
            for pun in puns:
                if pun['active_strike_count'] - diff >= 0:
                    ...
                    self.taskHandles.append(self.bot.loop.call_later(60 * 60 * 12, asyncio.create_task, self.expire_actions(
                        pun['_id'], ctx.guild.id)))  # Check in 12 hours, prevents time drifting
        ...
        if doc['type'] == 'strike':
            ...
            try:
                if user['strike_check'] > time.time():
                    ...
                    retryTime = twelveHr if user['strike_check'] - time.time(
                    ) > twelveHr else user['strike_check'] - time.time()
                    self.taskHandles.append(self.bot.loop.call_later(
                        retryTime, asyncio.create_task, self.expire_actions(_id, guild)))
                    return
            ...
            if doc['active_strike_count'] - 1 == 0:
                db.update_one({'_id': doc['_id']}, {
                              '$set': {'active': False}, '$inc': {'active_strike_count': -1}})
                strikes = [x for x in db.find(
                    {'user': doc['user'], 'type': 'strike', 'active': True}).sort('timestamp', 1)]
                ...
                db.update_one({'_id': doc['_id']}, {
                              '$inc': {'active_strike_count': -1}})
                self.taskHandles.append(self.bot.loop.call_later(
                    60 * 60 * 12, asyncio.create_task, self.expire_actions(doc['_id'], guild)))
            ...
            userDB.update_one({'_id': doc['user']}, {
                              '$set': {'strike_check': time.time() + 60 * 60 * 24 * 7}})

        elif doc['type'] == 'mute' and doc['expiry']:  # A mute that has an expiry
            # To prevent drift we recall every 12 hours. Schedule for 12hr or expiry time, whichever is sooner
            if doc['expiry'] > time.time():
                retryTime = twelveHr if doc['expiry'] - \
                    time.time() > twelveHr else doc['expiry'] - time.time()
                self.taskHandles.append(self.bot.loop.call_later(
                    retryTime, asyncio.create_task, self.expire_actions(_id, guild)))
                return
            ...
            try:
                ...
                self.taskHandles.append(self.bot.loop.call_later(
                    30, asyncio.create_task, self.expire_actions(_id, guild)))
                return
