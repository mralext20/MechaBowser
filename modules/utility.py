...
class ChatControl(commands.Cog, name='Utility Commands'):
    def __init__(self, bot):
        ...
        self.SMM2LevelID = re.compile(r'([0-9a-z]{3}-[0-9a-z]{3}-[0-9a-z]{3})',
                                      re.I | re.M)
    ...
    async def on_automod_finished(self, message):
        ...
        if msgInvites and config.moderator not in [
                x.id for x in message.author.roles
        ]:
        ...
        if message.channel.id == config.smm2Channel:
            ...
            embed.set_author(
                ...
                icon_url=
                f'{message.author.avatar_url}#mab_remover_{message.author.id}')
            embed.set_footer(
            ...
            for link in links:
                ...
                for i in range(0, len(labels)):
                    ...
                    if pathlib.PurePath(domain).match('amazon.*'):
                        match = re.match(r'^/exec/obidos/ASIN/(\w+)/.*$',
                                         urlParts.path)
                        if match:
                            linkModified = True
                            urlPartsList[
                                2] = f'/dp/{match.group(1)}'  # 2 = path
            ...
            if contentModified:
                ...
                if hooks:
                    ...
                    useHook = await message.channel.create_webhook(
                        ...
                        reason=
                        'No webhooks existed; 1 or more is required for affiliate filtering'
                    )
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    embed = discord.Embed(
                        description=
                        'The above message was automatically reposted by Mecha Bowser to remove an affiliate marketing link. The author may react with 🗑️ to delete these messages.'
                    )
                    ...
                    embed.set_footer(
                        text=
                        f'Author: {str(message.author)} ({message.author.id})',
                        icon_url=icon_url)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        for icon_url in [embed.author.icon_url, embed.footer.icon_url]:
            ...
            match = re.search(r'#mab_remover_(\d{15,25})(?:_(\d{15,25}))?$',
                              icon_url)
...
#        pass
    ...
    async def _clean(self, ctx, messages: int,
                     members: commands.Greedy[discord.Member]):
        if messages >= 100:

            def confirm_check(reaction, member):
                return member == ctx.author and str(
                    reaction.emoji) in [config.redTick, config.greenTick]

            confirmMsg = await ctx.send(
                f'This action will delete up to {messages}, are you sure you want to proceed?'
            )
            ...
            try:
                ...
                await confirmMsg.edit(
                    content='Confirmation timed out, clean action canceled.')
                return await confirmMsg.clear_reactions()
    ...
    async def _info(self, ctx, user: typing.Union[discord.Member, int]):
        ...
        if type(user) == int:
            ...
            if not dbUser:
                ...
                embed = discord.Embed(color=discord.Color(0x18EE1C),
                                      description=desc)
                ...
                embed.add_field(
                    name='Created',
                    value=user.created_at.strftime('%B %d, %Y %H:%M:%S UTC'))

                return await ctx.send(
                    embed=embed)  # TODO: Return DB info if it exists as well
        ...
        desc = f'Fetched user {user.mention}.' if inServer else (
        ...
        if inServer:
            embed.add_field(
                name='Join date',
                value=user.joined_at.strftime('%B %d, %Y %H:%M:%S UTC'),
                inline=True)
        roleList = []
        if inServer:
        ...
        lastMsg = 'N/a' if msgCount == 0 else datetime.datetime.utcfromtimestamp(
            messages.sort('timestamp', pymongo.DESCENDING)[0]
            ['timestamp']).strftime('%B %d, %Y %H:%M:%S UTC')
        ...
        if not punsCol.count():
            ...
            for pun in punsCol.sort('timestamp', pymongo.DESCENDING):
                ...
                if pun['type'] in [
                        'clear', 'unmute', 'unban', 'unblacklist', 'destrike'
                ]:
    ...
    async def _history(self,
                       ctx,
                       user: typing.Union[discord.User, int, None] = None):
        if user is None:
            user = ctx.author
        ...
        if ctx.guild.get_role(config.moderator
                              ) not in ctx.author.roles and ctx.guild.get_role(
                                  config.eh) not in ctx.author.roles:
            self_check = True
        ...
        deictic_language = {
            'no_punishments': ('User has no punishments on record',
                               'You have no available punishments on record'),
            'single_inf': ('There is **1** infraction record for this user:',
        ...
        if not puns.count():
            return await ctx.channel.send(
                f'{config.redTick} {deictic_language["no_punishments"][self_check]}'
            )
            ...
            ) == 1 else deictic_language['multiple_infs'][self_check].format(
                puns.count())
            fields = []
            activeStrikes = 0
            totalStrikes = 0
        ...
        try:
            ...
            await tools.send_paginated_embed(self.bot,
                                             channel,
                                             fields,
    ...
    async def _tag(self, ctx, *, query=None):
        ...
        if query:
            ...
            if not tag:
                return await ctx.send(
                    f'{config.redTick} A tag with that name does not exist',
                    delete_after=10)
    ...
    async def _tag_list(self, ctx, *, search: typing.Optional[str] = ''):
        ...
        if ctx.invoked_with in [
            ...
            if ctx.channel.id != config.commandsChannel:
                if not (ctx.guild.get_role(config.moderator)
                        in ctx.author.roles or ctx.guild.get_role(
                            config.helpfulUser) in ctx.author.roles):
                    return await ctx.send(
            ...
            if search:
                ...
                sort_joined_list.sort(
                    key=lambda e: e[0],
                    reverse=True)  # Sort from highest rank to lowest

                matches = list(
                    filter(lambda x: x[0] > 0,
                           sort_joined_list))  # Filter to those with matches

                tagList = [x[1] for x in matches]  # Resolve back to tags

            if tagList:
                longest_name = len(
                    max([tag['name'] for tag in tagList], key=len))
                lines = []

                for tag in tagList:
                    name = tag['name'].ljust(longest_name)
                    desc = '*No description*' if not tag['desc'] else tag[
                        'desc']
            ...
            return await tools.send_paginated_embed(self.bot,
                                                    ctx.channel,
                                                    fields,
                                                    owner=ctx.author,
                                                    title='Tag List',
    ...
    async def _tag_create(self, ctx, name, *, content):
        ...
        if name in [
                'list', 'search', 'edit', 'delete', 'source', 'setdesc',
                'setimg'
        ]:  # Name blacklist
            return await ctx.send(
                f'{config.redTick} You cannot use that name for a tag',
                delete_after=10)
        ...
        if tag:
            ...
            return await ctx.send(
                f'{config.greenTick} The **{name}** tag has been created',
                delete_after=10)
    ...
    async def _tag_delete(self, ctx, *, name):
        ...
        if tag:
            ...
            confirmMsg = await ctx.send(
                f'This action will delete the tag "{name}", are you sure you want to proceed?'
            )
            await confirmMsg.add_reaction(config.greenTick)
            await confirmMsg.add_reaction(config.redTick)
            try:
                ...
                await confirmMsg.edit(
                    content='Reaction timed out. Rerun command to try again')
                return await confirmMsg.clear_reactions()
                ...
                await confirmMsg.edit(
                    content=
                    f'{config.greenTick} The "{name}" tag has been deleted')
                await confirmMsg.clear_reactions()

        else:
            return await ctx.send(
                f'{config.redTick} The tag "{name}" does not exist')

    @_tag.command(name='setdesc')
    @commands.has_any_role(config.moderator, config.helpfulUser)
    async def _tag_setdesc(self,
                           ctx,
                           name,
                           *,
    ...
    async def _tag_setimg(self,
        ...
        if url and (
            ...
            return await ctx.send(
                f'{config.redTick} An invalid url, `{url}`, was given')
    ...
    async def _tag_source(self, ctx, *, name):
        ...
        if tag:
            embed = discord.Embed(title=f'{name} source',
                                  description=f'```md\n{tag["content"]}\n```')
            ...
            embed.add_field(
                name='Description',
                value='*No description*' if not description else description,
                inline=True)
            embed.add_field(name='Main Image',
                            value='*No URL set*' if not img_main else img_main,
                            inline=True)
            embed.add_field(
                name='Thumbnail Image',
                value='*No URL set*' if not img_thumb else img_thumb,
                inline=True)
    ...
    @commands.has_any_role(config.moderator, config.eh)
    async def _roles_set(self,
                         ctx,
                         member: discord.Member,
                         channel: typing.Union[discord.TextChannel,
                                               discord.CategoryChannel, str],
                         *,
        ...
        if type(channel) == str:
            ...
            if channel in ['mail', 'modmail']:
                ...
                return await ctx.send(
                    f'{config.redTick} You cannot blacklist a user from that function'
                )
            ...
            return await ctx.send(
                f'{config.redTick} You cannot blacklist a user from that channel'
            )
        ...
        try:
            ...
            await member.send(
                tools.format_pundm(statusText, reason, ctx.author, mention))

        except (discord.Forbidden,
                AttributeError):  # User has DMs off, or cannot send to Obj
            pass
        ...
        await ctx.send(
            f'{config.greenTick} {member} has been {statusText.lower()}ed from {mention}'
        )
    ...
    async def utility_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            return await ctx.send(
                f'{config.redTick} You are using that command too fast, try again in a few seconds',
                delete_after=15)
            ...
            return await ctx.send(
                f'{config.redTick} You do not have permission to run this command.',
                delete_after=15)
...
class AntiRaid(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.adminChannel = self.bot.get_channel(config.adminChannel)
        self.muteRole = self.bot.get_guild(config.nintendoswitch).get_role(
            config.mute)
        self.messages = {}
