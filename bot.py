...
try:
    ...
    logging.critical(
        '[Bot] config.py does not exist, you should make one from the example config')
    exit(1)
...
intents = discord.Intents(guilds=True, members=True, bans=True, emojis=True,
                          voice_states=True, presences=True, messages=True, reactions=True)
activityStatus = discord.Activity(
    type=discord.ActivityType.watching, name='over the server')
bot = commands.Bot(config.command_prefixes, intents=intents, max_messages=300000,
                   fetch_offline_members=True, activity=activityStatus, case_insensitive=True)
    ...
    async def on_ready(self):
        ...
        if not self.READY:
            ...
            for member in NS.members:
                userCount += 1
                logging.debug(
                    f'[Cache] Syncronizing user {userCount}/{guildCount}')
