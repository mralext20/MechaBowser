...
class Moderation(commands.Cog, name='Moderation Commands'):
    ...
    async def _hide_modlog(self, ctx, uuid):
        ...
        if not doc:
            return await ctx.send(
                f'{config.redTick} No punishment with that UUID exists')

        sensitive = True if not doc[
            'sensitive'] else False  # Toggle sensitive value
        ...
        if not doc['public_log_message']:
            ...
            for field in embedDict['fields']:
                ...
                if field[
                        'name'] == 'Reason':  # This is subject to a breaking change if `name` updated, but I'll take the risk
                    if sensitive:
                        ...
                        newEmbedDict['fields'][listIndex]['value'] = doc[
                            'reason']
            ...
            assert embedDict['fields'] != newEmbedDict[
                'fields']  # Will fail if message was unchanged, this is likely because of a breaking change upstream in the pun flow
            db.update_one({'_id': uuid}, {'$set': {'sensitive': sensitive}})
    ...
    async def _banning(self,
        ...
        for user in users:
            ...
            user = discord.Object(id=userid) if (
                type(user) is int
            ) else user  # If not a user, manually contruct a user object
            try:
    ...
    async def _muting(self,
        ...
        try:
            ...
            await ctx.send(
                f'{config.greenTick} {member} ({member.id}) has been successfully muted'
            )
    ...
    async def _warning(self, ctx):
        await ctx.send(
            ':warning: Warns are depreciated. Please use the strike system instead (`!help strike`)'
        )
    ...
    async def _inf_revoke(self, ctx, _id):
        ...
        await ctx.send(
            f'{config.greenTick} removed {_id}: {doc["type"]} against {doc["user"]} by {doc["moderator"]}'
        )
